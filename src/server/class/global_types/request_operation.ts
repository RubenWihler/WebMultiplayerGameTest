...
import ObservableEvent from "../event_system/observable_event";
...
export default class RequestOperation<SendType, ResponseType> {
    ...
    constructor(socket: Socket, send_message_name: string, receive_message_name: string, send_data: SendType, timeout: number = 10000,
    ...
    public async start() : Promise<ResponseType> {
        ...
        this.socket.emit(this.send_message_name, this.send_data);

        //bind response event
        this.socket.on(this.receive_message_name, (data: ResponseType) => {
